...
new("cloutput", y=y, yhat=yhat, learnind = learnind,
     prob = pred.test, method = "rf", mode=mode)
}
})
...
setMethod("rfCMA", signature(X="matrix", y="factor", f="missing"),
          function(X, y, learnind, varimp=TRUE, seed=111, ...){
rfCMA(X, y=as.numeric(y)-1, learnind=learnind, varimp=varimp, seed=seed, ...)
})
...
rfCMA(as.matrix(X), y=y, learnind=learnind, varimp = varimp, seed = seed, ...)})
...
setMethod("rfCMA", signature(X="ExpressionSet", y="character", f="missing"),
          ...
          rfCMA(X=X, y=y, learnind=learnind, varimp=varimp, seed=seed, ...)})